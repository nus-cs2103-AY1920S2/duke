...
For now, let’s have the application add a new `Label` with the text from the `TextField`. Update the `duke.gui.Main` class as follows. You'll need to add an `import javafx.scene.control.Label;` too.
```java
@Override
public void start(Stage stage) {
    ...
    //Step 3. duke.command.Add functionality to handle user input.
...
We use the code in our main class just like any other control. Here are the steps to update the code to use the custom control in `duke.gui.Main.java`.
...
duke.command.Add a new method to handle user input:
```java
/**
 * Iteration 2:
...
public void start(Stage stage) {
    ...
    //Part 3. duke.command.Add functionality to handle user input.
    sendButton.setOnMouseClicked((event) -> {
        handleUserInput();
    });
...
Now, we can go back to the `duke.gui.Main` class and change the event handler to use our new `duke.gui.DialogBox`. 
...
1. While the GUI looks similar to the mockup, there are still parts that need to be refined. Try your hand at some of these tasks:
   * duke.command.Add padding between each duke.gui.DialogBox
   * duke.command.Add padding between each ImageView and its Label
   * Clip the ImageView into a circle
   * duke.command.Add background color to each dialog box
