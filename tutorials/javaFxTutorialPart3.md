...
For dukeApp.duke.Duke, there are two events that we want to respond to, namely the user pressing `Enter` in the `TextField` and left-clicking the `Button`. These are the `onAction` event for the `TextField` and the `onMouseClicked` event for the `Button`.

For now, let’s have the application add a new `Label` with the text from the `TextField`. Update the `dukeApp.duke.Main` class as follows. You'll need to add an `import javafx.scene.control.Label;` too.
```java
@Override
...
Let’s create our custom control `dukeApp.duke.DialogBox`:
```java
...
public class dukeApp.duke.DialogBox extends HBox {
    ...
    public dukeApp.duke.DialogBox(Label l, ImageView iv) {
        text = l;
        displayPicture = iv;
...
We use the code in our main class just like any other control. Here are the steps to update the code to use the custom control in `dukeApp.duke.Main.java`.
...
For this tutorial, we have two images `DaUser.png` and `DaDuke.png` to represent the user avatar and dukeApp.duke.Duke's avatar respectively but you can use any image you want.
...
```java
public class dukeApp.duke.Duke extends Application {
    // ...
    private Image user = new Image(this.getClass().getResourceAsStream("/images/DaUser.png"));
    private Image dukeApp.duke = new Image(this.getClass().getResourceAsStream("/images/DaDuke.png"));
    // ...
}
```
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            new dukeApp.duke.DialogBox(userText, new ImageView(user)),
            new dukeApp.duke.DialogBox(dukeText, new ImageView(dukeApp.duke))
    );
    userInput.clear();
}
...
## Iteration 3 – Adding custom behavior to dukeApp.duke.DialogBox

One additional benefit of defining a custom control is that we can add behavior specific to our `dukeApp.duke.DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and dukeApp.duke.Duke’s output.
...
public static dukeApp.duke.DialogBox getUserDialog(Label l, ImageView iv) {
    return new dukeApp.duke.DialogBox(l, iv);
}

public static dukeApp.duke.DialogBox getDukeDialog(Label l, ImageView iv) {
    var db = new dukeApp.duke.DialogBox(l, iv);
    db.flip();
    return db;
}
```
...
Now, we can go back to the `dukeApp.duke.Main` class and change the event handler to use our new `dukeApp.duke.DialogBox`. 
...
private void handleUserInput() {
    ...
    Label dukeText = new Label(getResponse(userInput.getText()));
    dialogContainer.getChildren().addAll(
            dukeApp.duke.DialogBox.getUserDialog(userText, new ImageView(user)),
            dukeApp.duke.DialogBox.getDukeDialog(dukeText, new ImageView(dukeApp.duke))
    );
    userInput.clear();
}
```
...
Congratulations! 
You have successfully implemented a fully functional GUI for dukeApp.duke.Duke!  
...
1. While the GUI looks similar to the mockup, there are still parts that need to be refined. Try your hand at some of these tasks:
   * Add padding between each dukeApp.duke.DialogBox
