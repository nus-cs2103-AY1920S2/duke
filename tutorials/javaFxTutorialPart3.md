...
Let’s create our custom control `ip.view.DialogBox`:
```java
...
public class ip.view.DialogBox extends HBox {
    ...
    public ip.view.DialogBox(Label l, ImageView iv) {
        text = l;
        displayPicture = iv;
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            new ip.view.DialogBox(userText, new ImageView(user)),
            new ip.view.DialogBox(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
...
## Iteration 3 – Adding custom behavior to ip.view.DialogBox

One additional benefit of defining a custom control is that we can add behavior specific to our `ip.view.DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and Duke’s output.
...
public static ip.view.DialogBox getUserDialog(Label l, ImageView iv) {
    return new ip.view.DialogBox(l, iv);
}

public static ip.view.DialogBox getDukeDialog(Label l, ImageView iv) {
    var db = new ip.view.DialogBox(l, iv);
    db.flip();
    return db;
}
```
...
Now, we can go back to the `Main` class and change the event handler to use our new `ip.view.DialogBox`. 
...
private void handleUserInput() {
    ...
    Label dukeText = new Label(getResponse(userInput.getText()));
    dialogContainer.getChildren().addAll(
            ip.view.DialogBox.getUserDialog(userText, new ImageView(user)),
            ip.view.DialogBox.getDukeDialog(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
```
...
1. While the GUI looks similar to the mockup, there are still parts that need to be refined. Try your hand at some of these tasks:
   * Add padding between each ip.view.DialogBox
