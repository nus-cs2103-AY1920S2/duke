...
For dukeproj.Duke, there are two events that we want to respond to, namely the user pressing `Enter` in the `TextField` and left-clicking the `Button`. These are the `onAction` event for the `TextField` and the `onMouseClicked` event for the `Button`.
...
For this tutorial, we have two images `DaUser.png` and `DaDuke.png` to represent the user avatar and dukeproj.Duke's avatar respectively but you can use any image you want.
...
```java
public class dukeproj.Duke extends Application {
    // ...
    private Image user = new Image(this.getClass().getResourceAsStream("/images/DaUser.png"));
    private Image dukeproj = new Image(this.getClass().getResourceAsStream("/images/DaDuke.png"));
    // ...
}
```
...
/**
 * Iteration 2:
 * Creates two dialog boxes, one echoing user input and the other containing dukeproj.Duke's reply and then appends them to
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            ...
            new DialogBox(dukeText, new ImageView(dukeproj))
    );
    userInput.clear();
}
...
private String getResponse(String input) {
    return "dukeproj.Duke heard: " + input;
}
```
...
One additional benefit of defining a custom control is that we can add behavior specific to our `DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and dukeproj.Duke’s output.
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            ...
            DialogBox.getDukeDialog(dukeText, new ImageView(dukeproj))
    );
    userInput.clear();
}
```
...
Congratulations! 
You have successfully implemented a fully functional GUI for dukeproj.Duke!  
