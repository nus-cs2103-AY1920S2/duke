...
Let’s create our custom control `duke.window.DialogBox`:
```java
...
public class duke.window.DialogBox extends HBox {
    ...
    public duke.window.DialogBox(Label l, ImageView iv) {
        text = l;
        displayPicture = iv;
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            new duke.window.DialogBox(userText, new ImageView(user)),
            new duke.window.DialogBox(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
...
## Iteration 3 – Adding custom behavior to duke.window.DialogBox

One additional benefit of defining a custom control is that we can add behavior specific to our `duke.window.DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and duke.Duke’s output.
...
public static duke.window.DialogBox getUserDialog(Label l, ImageView iv) {
    return new duke.window.DialogBox(l, iv);
}

public static duke.window.DialogBox getDukeDialog(Label l, ImageView iv) {
    var db = new duke.window.DialogBox(l, iv);
    db.flip();
    return db;
}
```
...
Now, we can go back to the `duke.Main` class and change the event handler to use our new `duke.window.DialogBox`. 
...
private void handleUserInput() {
    ...
    Label dukeText = new Label(getResponse(userInput.getText()));
    dialogContainer.getChildren().addAll(
            duke.window.DialogBox.getUserDialog(userText, new ImageView(user)),
            duke.window.DialogBox.getDukeDialog(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
```
...
1. While the GUI looks similar to the mockup, there are still parts that need to be refined. Try your hand at some of these tasks:
   * Add padding between each duke.window.DialogBox
