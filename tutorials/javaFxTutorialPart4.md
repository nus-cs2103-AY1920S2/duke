...
Let's return to duke.duke.Duke and convert it to use FXML instead.
...
<AnchorPane maxHeight="-Infinity" maxWidth="-Infinity" minHeight="-Infinity" minWidth="-Infinity" prefHeight="600.0" prefWidth="400.0" xmlns="http://javafx.com/javafx/8.0.171" xmlns:fx="http://javafx.com/fxml/1" fx:controller="duke.javafx.MainWindow">
  <children>
    <TextField fx:id="userInput" layoutY="558.0" onAction="#handleUserInput" prefHeight="41.0" prefWidth="324.0" AnchorPane.bottomAnchor="1.0" />
...
1. Let’s explore the provided FXML files in Scene Builder. 
    ...
    Select `Open Project` > `src/main/resources/view/duke.javafx.MainWindow.fxml`. Inspect each control and its properties.

   ![SceneBuilder opening duke.javafx.MainWindow.fxml](assets/SceneBuilder.png)
...
1. On the left accordion, you can see that we have set the controller class to `duke.javafx.MainWindow`. 
We will get to that later.
 
   ![Controller for duke.javafx.MainWindow](assets/MainWindowController.png)
...
As part of the effort to separate the code handling duke.duke.Duke's logic and UI, let's _refactor_ the UI-related code to its own class.
We call these UI classes _controllers_. 

Let's implement the `duke.javafx.MainWindow` controller class that we specified in `duke.javafx.MainWindow.fxml`.

**duke.javafx.MainWindow.java**
```java
 ...
 */
public class duke.javafx.MainWindow extends AnchorPane {
    @FXML
    ...
    public void setDuke(duke.duke.Duke d) {
        duke = d;
    }
...
Notice how in `duke.javafx.MainWindow`, we can invoke `TextField#clear()` on `userInput` and access its content just as we did in the previous example.
...
Let's create a new `Main` class as the bridge between the existing logic in `duke.duke.Duke` and the UI in `duke.javafx.MainWindow`.
...
public class Main extends Application {
    ...
    private duke.duke.Duke duke = new duke.duke.Duke();

    @Override
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(Main.class.getResource("/view/duke.javafx.MainWindow.fxml"));
            AnchorPane ap = fxmlLoader.load();
            Scene scene = new Scene(ap);
            stage.setScene(scene);
            fxmlLoader.<duke.javafx.MainWindow>getController().setDuke(duke);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
The last change that we have to make is to point our `duke.javafx.Launcher` class in the right direction:
In `duke.javafx.Launcher.java`
```java
//...    
...
1. Convert `duke.javafx.MainWindow` to use the `fx:root` construct.
1. Extend `duke.javafx.MainWindow` to have a `Stage` as a root Node.
