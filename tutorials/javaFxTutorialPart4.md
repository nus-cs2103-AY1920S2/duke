...
1. Components are heavily dependent on each other:
   Why does `duke.main.Main` need to know that `duke.main.DialogBox` needs a `Label`? 
...
1. The code is untidy and long:
   ...
   The `duke.main.Main` class attempts to do it all. 
...
Let's return to duke.main.Duke and convert it to use FXML instead.
...
<AnchorPane maxHeight="-Infinity" maxWidth="-Infinity" minHeight="-Infinity" minWidth="-Infinity" prefHeight="600.0" prefWidth="400.0" xmlns="http://javafx.com/javafx/8.0.171" xmlns:fx="http://javafx.com/fxml/1" fx:controller="duke.main.MainWindow">
  <children>
    <TextField fx:id="userInput" layoutY="558.0" onAction="#handleUserInput" prefHeight="41.0" prefWidth="324.0" AnchorPane.bottomAnchor="1.0" />
...
1. Let’s explore the provided FXML files in Scene Builder. 
    ...
    Select `Open Project` > `src/main/resources/view/duke.main.MainWindow.fxml`. Inspect each control and its properties.

   ![SceneBuilder opening duke.main.MainWindow.fxml](assets/SceneBuilder.png)
...
1. On the left accordion, you can see that we have set the controller class to `duke.main.MainWindow`. 
We will get to that later.
 
   ![Controller for duke.main.MainWindow](assets/MainWindowController.png)

1. Let’s repeat the process for `duke.main.DialogBox`.
   The main difference here is that duke.main.DialogBox checks `Use fx:root construct` and _does not define a controller class_. 

   ![Settings for duke.main.DialogBox](assets/DialogBoxController.png)

## Using Controllers

As part of the effort to separate the code handling duke.main.Duke's logic and UI, let's _refactor_ the UI-related code to its own class.
We call these UI classes _controllers_. 

Let's implement the `duke.main.MainWindow` controller class that we specified in `duke.main.MainWindow.fxml`.

**duke.main.MainWindow.java**
```java
 ...
 */
public class duke.main.MainWindow extends AnchorPane {
    @FXML
    ...
    public void setDuke(duke.main.Duke d) {
        duke = d;
    }
    ...
    private void handleUserInput() {
        ...
        dialogContainer.getChildren().addAll(
                duke.main.DialogBox.getUserDialog(input, userImage),
                duke.main.DialogBox.getDukeDialog(response, dukeImage)
        );
        userInput.clear();
    }
}
...
Notice how in `duke.main.MainWindow`, we can invoke `TextField#clear()` on `userInput` and access its content just as we did in the previous example.
...
Let's create a new `duke.main.Main` class as the bridge between the existing logic in `duke.main.Duke` and the UI in `duke.main.MainWindow`.
 ...
 */
public class duke.main.Main extends Application {

    private duke.main.Duke duke = new duke.main.Duke();

    @Override
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(duke.main.Main.class.getResource("/view/duke.main.MainWindow.fxml"));
            AnchorPane ap = fxmlLoader.load();
            Scene scene = new Scene(ap);
            stage.setScene(scene);
            fxmlLoader.<duke.main.MainWindow>getController().setDuke(duke);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
For our custom `duke.main.DialogBox`, we did not define a controller so let's create a controller for it.

**duke.main.DialogBox.java**
```java
import java.io.IOException;
 ...
 */
public class duke.main.DialogBox extends HBox {
    @FXML
    private Label dialog;
    @FXML
    private ImageView displayPicture;

    private duke.main.DialogBox(String text, Image img) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(duke.main.MainWindow.class.getResource("/view/duke.main.DialogBox.fxml"));
    ...
    public static duke.main.DialogBox getUserDialog(String text, Image img) {
        return new duke.main.DialogBox(text, img);
    }

    public static duke.main.DialogBox getDukeDialog(String text, Image img) {
        var db = new duke.main.DialogBox(text, img);
        db.flip();
        return db;
    }
}
...
When we create a new instance of `duke.main.DialogBox`, we set both the controller and root Node to `duke.main.DialogBox`. 
From this point onwards we can interact with `duke.main.DialogBox` as we have in the previous tutorials.
...
1. Convert `duke.main.MainWindow` to use the `fx:root` construct.
1. Extend `duke.main.MainWindow` to have a `Stage` as a root Node.
