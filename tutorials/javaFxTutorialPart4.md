...
1. Components are heavily dependent on each other:
   Why does `duke.Main` need to know that `duke.window.DialogBox` needs a `Label`? 
...
<AnchorPane maxHeight="-Infinity" maxWidth="-Infinity" minHeight="-Infinity" minWidth="-Infinity" prefHeight="600.0" prefWidth="400.0" xmlns="http://javafx.com/javafx/8.0.171" xmlns:fx="http://javafx.com/fxml/1" fx:controller="duke.window.MainWindow">
  <children>
    <TextField fx:id="userInput" layoutY="558.0" onAction="#handleUserInput" prefHeight="41.0" prefWidth="324.0" AnchorPane.bottomAnchor="1.0" />
...
1. Let’s explore the provided FXML files in Scene Builder. 
    ...
    Select `Open Project` > `src/main/resources/view/duke.window.MainWindow.fxml`. Inspect each control and its properties.

   ![SceneBuilder opening duke.window.MainWindow.fxml](assets/SceneBuilder.png)
...
1. On the left accordion, you can see that we have set the controller class to `duke.window.MainWindow`. 
We will get to that later.
 
   ![Controller for duke.window.MainWindow](assets/MainWindowController.png)

1. Let’s repeat the process for `duke.window.DialogBox`.
   The main difference here is that duke.window.DialogBox checks `Use fx:root construct` and _does not define a controller class_. 

   ![Settings for duke.window.DialogBox](assets/DialogBoxController.png)
...
Let's implement the `duke.window.MainWindow` controller class that we specified in `duke.window.MainWindow.fxml`.

**duke.window.MainWindow.java**
```java
...
import javafx.scene.layout.VBox;
/**
 * Controller for duke.window.MainWindow. Provides the layout for the other controls.
 */
public class duke.window.MainWindow extends AnchorPane {
    @FXML
    ...
    private void handleUserInput() {
        ...
        dialogContainer.getChildren().addAll(
                duke.window.DialogBox.getUserDialog(input, userImage),
                duke.window.DialogBox.getDukeDialog(response, dukeImage)
        );
        userInput.clear();
    }
}
...
Notice how in `duke.window.MainWindow`, we can invoke `TextField#clear()` on `userInput` and access its content just as we did in the previous example.
...
Let's create a new `duke.Main` class as the bridge between the existing logic in `duke.Duke` and the UI in `duke.window.MainWindow`.
...
public class duke.Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(duke.Main.class.getResource("/view/duke.window.MainWindow.fxml"));
            AnchorPane ap = fxmlLoader.load();
            Scene scene = new Scene(ap);
            stage.setScene(scene);
            fxmlLoader.<duke.window.MainWindow>getController().setDuke(duke);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
For our custom `duke.window.DialogBox`, we did not define a controller so let's create a controller for it.

**duke.window.DialogBox.java**
```java
import java.io.IOException;
 ...
 */
public class duke.window.DialogBox extends HBox {
    @FXML
    private Label dialog;
    @FXML
    private ImageView displayPicture;

    private duke.window.DialogBox(String text, Image img) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(duke.window.MainWindow.class.getResource("/view/duke.window.DialogBox.fxml"));
    ...
    public static duke.window.DialogBox getUserDialog(String text, Image img) {
        return new duke.window.DialogBox(text, img);
    }

    public static duke.window.DialogBox getDukeDialog(String text, Image img) {
        var db = new duke.window.DialogBox(text, img);
        db.flip();
        return db;
    }
}
...
When we create a new instance of `duke.window.DialogBox`, we set both the controller and root Node to `duke.window.DialogBox`. 
From this point onwards we can interact with `duke.window.DialogBox` as we have in the previous tutorials.
...
1. Convert `duke.window.MainWindow` to use the `fx:root` construct.
1. Extend `duke.window.MainWindow` to have a `Stage` as a root Node.
