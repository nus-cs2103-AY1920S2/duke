...
1. Let’s explore the provided FXML files in Scene Builder. 
    ...
    Select `Open Project` > `src/main/resources/view/duke.ui.MainWindow.fxml`. Inspect each control and its properties.
...
Let's create a new `Main` class as the bridge between the existing logic in `Duke` and the UI in `duke.ui.MainWindow`.
...
public class Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(Main.class.getResource("/view/duke.ui.MainWindow.fxml"));
