...
1. Components are heavily dependent on each other:
   Why does `Main` need to know that `DialogBoxOld` needs a `Label`? 
...
1. Let’s repeat the process for `DialogBoxOld`.
   The main difference here is that DialogBoxOld checks `Use fx:root construct` and _does not define a controller class_. 

   ![Settings for DialogBoxOld](assets/DialogBoxController.png)
...
public class MainWindow extends AnchorPane {
    ...
    private void handleUserInput() {
        ...
        dialogContainer.getChildren().addAll(
                DialogBoxOld.getUserDialog(input, userImage),
                DialogBoxOld.getDukeDialog(response, dukeImage)
        );
        userInput.clear();
    }
}
...
For our custom `DialogBoxOld`, we did not define a controller so let's create a controller for it.
...
public class DialogBoxOld extends HBox {
    ...
    private DialogBoxOld(String text, Image img) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(MainWindow.class.getResource("/view/DialogBoxOld.fxml"));
    ...
    public static DialogBoxOld getUserDialog(String text, Image img) {
        return new DialogBoxOld(text, img);
    }

    public static DialogBoxOld getDukeDialog(String text, Image img) {
        var db = new DialogBoxOld(text, img);
        db.flip();
        return db;
    }
}
...
When we create a new instance of `DialogBoxOld`, we set both the controller and root Node to `DialogBoxOld`. 
From this point onwards we can interact with `DialogBoxOld` as we have in the previous tutorials.
