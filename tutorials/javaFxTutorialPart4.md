...
Let's return to Driver.Duke and convert it to use FXML instead.
...
As part of the effort to separate the code handling Driver.Duke's logic and UI, let's _refactor_ the UI-related code to its own class.
...
**MainWindow.java**
```java
import driver.Duke;import javafx.fxml.FXML;
import javafx.scene.control.Button;
import javafx.scene.control.ScrollPane;
import javafx.scene.control.TextField;
...
Let's create a new `Main` class as the bridge between the existing logic in `Driver.Duke` and the UI in `MainWindow`.
...
/**
 * A GUI for Driver.Duke using FXML.
 */
