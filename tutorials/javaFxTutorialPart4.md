...
1. Components are heavily dependent on each other:
   Why does `duke.gui.Main` need to know that `duke.gui.DialogBox` needs a `Label`? 
...
1. The code is untidy and long:
   ...
   The `duke.gui.Main` class attempts to do it all. 
...
Let's create a new `duke.gui.Main` class as the bridge between the existing logic in `duke.Duke` and the UI in `duke.gui.MainWindow`.
 ...
 */
public class duke.gui.Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(duke.gui.Main.class.getResource("/view/duke.gui.MainWindow.fxml"));
...
```java
//...    
Application.launch(duke.gui.Main.class, args);
//...
```
