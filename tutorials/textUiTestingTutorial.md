...
1. duke.command.Add a `runtest.bat` (if you are on Windows) or `runtest.sh` (if you are on a *nix OS) into the folder, containing the script below.<br>
  `runtest.bat`:
   ```
   @ECHO OFF
...
1. duke.command.Add an `EXPECTED.txt` to the same folder, containing the expected output.
1. duke.command.Add an `input.txt` containing the input commands.
